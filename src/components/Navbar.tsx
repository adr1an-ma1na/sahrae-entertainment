...
export default function Navbar({ activeTab, setActiveTab, onSearch, onPlay }: NavbarProps) {
  ...
  return (
    <>
    <nav className={`fixed top-0 left-0 right-0 md:left-16 lg:left-64 z-40 transition-all duration-500 pt-[env(safe-area-inset-top)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)] ${isScrolled ? 'glass border-b border-white/15' : 'bg-gradient-to-b from-black/55 via-black/20 to-transparent backdrop-blur-sm'}`}>
