      ...
      <div className="relative w-full sm:max-w-md glass rounded-t-3xl sm:rounded-3xl border border-white/10 shadow-2xl max-h-[80vh] flex flex-col animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex items-center gap-3 p-4 border-b border-white/10">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] text-amber-400 font-semibold uppercase tracking-wide">Add to playlist</p>
        ...
        <div className="p-4 border-b border-white/5">
          {creating ? (
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleCreate} className="btn-gold px-4 py-2.5 rounded-xl text-sm font-bold">Create</button>
            </div>
          ) : (
            <button onClick={() => setCreating(true)} className="flex items-center gap-3 w-full text-left text-white">
              <span className="w-11 h-11 rounded-xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center"><Plus className="w-5 h-5 text-amber-400" /></span>
            ...
            playlists.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => { if (!has) addToPlaylist(p.id, track); closeAddSheet(); }}
                  ...
                  {has && <Check className="w-5 h-5 text-amber-400 shrink-0" />}
                </button>
              );
            })
          )}
