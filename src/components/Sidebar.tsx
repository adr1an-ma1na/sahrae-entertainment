...
export default function Sidebar({ activeTab, setActiveTab, collapsed = false, onToggle }: { activeTab: string; setActiveTab: (t: string) => void; collapsed?: boolean; onToggle?: () => void }) {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex lg:hidden flex-col items-center fixed left-0 top-0 bottom-0 w-16 z-40 glass-liquid border-r border-white/12 pt-[calc(env(safe-area-inset-top)+0.75rem)] pb-3 overflow-y-auto scrollbar-hide">
      ...
      <aside className={`hidden lg:flex flex-col fixed left-0 top-0 bottom-0 w-64 z-40 glass-liquid border-r border-white/15 shadow-[inset_-1px_0_0_rgba(255,255,255,0.12),12px_0_48px_rgba(0,0,0,0.45)] pt-[env(safe-area-inset-top)] transition-transform duration-300 ${collapsed ? '-translate-x-full' : 'translate-x-0'}`}>
